<?php



	$title= 'Blood Donation Request Page';
	include '../base.php';
	
?>
	<?php include "top.php"?>
	<div class="main p-3 container">
		<h3 class="text-center">Make a request</h3>
		<div class="col-8" style="margin: 0 auto;">
			<form method = "POST">
				<div class="row">
					<div class="col-12">
					    <label for="inputBloodAmount" class="">Blood Amount(ml)</label>
						<input type="number" id="inputBloodAmount" class="form-control" name="amount" placeholder="Enter a number from 200-550" min="200" max="550" required>
					</div>
					<div class="col-12">
						<label for="inputDate" class="">How soon do you need it?</label>
						<input type="date"  name="date_needed" id="inputDate" class="form-control" required>
					</div>
					<div class="col-12">
					    <label for="inputPurpose" class="">Purpose</label>
						<textarea id="inputPurpose" class="form-control" name="purpose" required></textarea>
					</div>
				</div>
			    <button class="w-100 mt-2 btn btn-primary btn-lg" name="triggers" value="request_blood" type="submit">Submit</button>
			</form>
		</div>
	</div>
	<?php include 'buttom.php';